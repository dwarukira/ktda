...
const TabItems = ({ children, loading, currentItem, people }: any) => {
  ...
  const [ulPos, setUlPos] = useState<DOMRect>();
  // Tab sizes
  const [tabs, setTabs] = React.useState<DOMRect[]>();
  ...
  const getSize = () => {
    if (ulRef.current) {
      const tabList = Array.from(ulRef.current.children).map((i) => {
  ...
  return (
    <Tabs>
      <ul ref={ulRef}>
        ...
        {tabs && ulPos ? (
          <StyledIndicator
            width={tabs[current].width}
            left={tabs[current].left - ulPos.left}
          />
        ) : (
          ""
        )}
